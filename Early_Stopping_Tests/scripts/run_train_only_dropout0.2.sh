...
#
#   sed -i 's/\r$//' Early_Stopping_Tests/scripts/run_train_only_dropout0.2.sh   # if edited on Windows
#   sbatch Early_Stopping_Tests/scripts/run_train_only_dropout0.2.sh
...
# Resolve paths relative to this script so the folder can live anywhere.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
EARLY_STOPPING_TESTS_DIR="${EARLY_STOPPING_TESTS_DIR:-$(dirname "$SCRIPT_DIR")}"
...
stage_start=$SECONDS
python "$EARLY_STOPPING_TESTS_DIR/Python Files/train_with_early_stopping.py" \
